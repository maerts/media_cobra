...
/**
 *  @file
 *  Create the 'Cobra' tab for the WYSIWYG plugins.
 */

(function ($) {
  namespace('media.browser.plugin');

  Drupal.media.browser.plugin.cobra_library = function(mediaBrowser, options) {

    return {
      mediaFiles: [],
      init: function() {
        tabset = mediaBrowser.getTabset();
        tabset.tabs('add', '#cobra_library', 'Cobra');
        var that = this;
  ...
  Drupal.media.browser.register('cobra_library', Drupal.media.browser.plugin.cobra_library);
})(jQuery);
