...
/**
 * @file media_cobra/js/media_cobra.js
 */

(function ($) {

Drupal.media_cobra = {};
Drupal.behaviors.media_cobra = {
  attach: function (context, settings) {
    ...
    if (!FlashDetect.installed && !html5){
      $('.media-cobra-preview-wrapper').each(Drupal.media_cobra.needFlash);
    }
  }
};

Drupal.media_cobra.needFlash = function () {
  var id = $(this).attr('id');
  var wrapper = $('.media-cobra-preview-wrapper');
  var hw = Drupal.settings.media_cobra[id].height / Drupal.settings.media_cobra[id].width;
...
Drupal.media_cobra.insertEmbed = function (embed_id) {
  var videoWrapper = $('#' + embed_id + '.media-cobra-preview-wrapper');
  var settings = Drupal.settings.media_cobra[embed_id];

  // Calculate the ratio of the dimensions of the embed.
  settings.hw = settings.height / settings.width;

  // Replace the object embed with Cobra's iframe. This isn't done by the
  // theme function because Cobra doesn't have a no-JS or no-Flash fallback.
  var video = $('<iframe class="cobra-player" type="text/html" frameborder="0"></iframe>');
  var src = 'http://www.cobra.be/permalink/' + settings.video_id;

  // Allow other modules to modify the video settings.
  settings.options.wmode = 'opaque';
  $(window).trigger('media_cobra_load', settings);

  // Merge Cobra options (such as autoplay) into the source URL.
  ...
  $(window).bind('resize', Drupal.media_cobra.resizeEmbeds);
  ...
  if (!$('.lightbox-stack').length) {
    setTimeout(Drupal.media_cobra.resizeEmbeds, 1);
  }
};

Drupal.media_cobra.resizeEmbeds = function () {
  $('.media-cobra-preview-wrapper').each(Drupal.media_cobra.resizeEmbed);
};

Drupal.media_cobra.resizeEmbed = function () {
  var context = $(this).parent();
  var video = $(this).children(':first-child');
  var hw = Drupal.settings.media_cobra[$(this).attr('id')].hw;
  // Change the height of the wrapper that was given a fixed height by the
  // Cobra theming function.
  $(this)
